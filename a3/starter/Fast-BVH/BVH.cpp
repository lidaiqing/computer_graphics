...
bool BVH::getIntersection(const Ray& ray, IntersectionInfo* intersection, int index, bool occlusion) const {
  ...
  while(stackptr>=0) {
    ...
    if( node.rightOffset == 0 ) {
      for(uint32_t o=0;o<node.nPrims;++o) {
        ...
        if (obj->getIndex() == index) continue; // do not intersect with itself
        if (hit) {
