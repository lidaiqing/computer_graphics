...
float distanceBoids(int i, int j);
int* randSelect(int n);
// ******************** FUNCTIONS ************************
...
int main(int argc, char** argv)
{
    ...
    leader_list = randSelect(MAX_LEADERS);
...
//
      ...
      GLUI_Spinner *global_rot_spinner
          = glui->add_spinner("rotation", GLUI_SPINNER_FLOAT, &global_rot);
...
//    Min and max values ------------|--|
      ...
      k0_spinner->set_speed(0.1);
      k0_spinner->set_float_limits(0.001, 0.999, GLUI_LIMIT_CLAMP);
      k1_spinner->set_speed(0.1);
      k1_spinner->set_float_limits(0, 1, GLUI_LIMIT_CLAMP);
      k2_spinner->set_speed(0.1);
      k2_spinner->set_float_limits(0, 1, GLUI_LIMIT_CLAMP);
      k3_spinner->set_speed(0.1);
...
/***** Scene drawing start *********/
    ...
    // listen for the change of global_rot
    WindowReshape(Win[0], Win[1]);
...
{
 /*
   This function calculate distance between two boids
 */
	float i_pos[3] = {Boid_Location[i][0], Boid_Location[i][1], Boid_Location[i][2]};
	float j_pos[3] = {Boid_Location[j][0], Boid_Location[j][1], Boid_Location[j][2]};
	float dist = 0;
	for (int k = 0; k < 3; k++)
	{
		dist += (i_pos[k] - j_pos[k]) * (i_pos[k] - j_pos[k]);
	}
	dist = sqrt(dist);
	return dist;
}
...
{
/*
  Randomly select n numbers from 0 to nBoids
*/
	srand(time(0));
	int* nums = (int*)malloc(n * sizeof(int));
	for (int i = 0; i < n; i++)
	{
		int rnum = rand() % nBoids;
		int j;
		for (j = 0; j < i; j++)
		{
			if (nums[j] == rnum) break;
		}
		if (j == i) nums[i] = rnum;
		else i--;
	}
 ...
 */
 ...
 // hours.
 ///////////////////////////////////////////
	float velocity_before_update[3] = {Boid_Velocity[i][0], Boid_Velocity[i][1], Boid_Velocity[i][2]};
...
// applying rule 1
	...
	int num_of_near_boids = 0;
	for (int j = 0; j < nBoids; j++)
	{
		if (j == i || distanceBoids(i, j) > r_rule1) continue;		
		num_of_near_boids++;
		for (int k = 0; k < 3; k++)
			center_of_mass[k] += Boid_Location[j][k];
	}
	for (int k = 0; k < 3; k++)
		if (num_of_near_boids != 0)
			center_of_mass[k] /= (num_of_near_boids);
	float V1[3];
	for (int k = 0; k < 3; k++)
		V1[k] = center_of_mass[k] - Boid_Location[i][k];
	// update boid i velocity
	for (int k = 0; k < 3; k++)
		Boid_Velocity[i][k] += k_rule1 * V1[k];
 ///////////////////////////////////////////
 // QUESTION:
...
///////////////////////////////////////////
 ...
 ///////////////////////////////////////////
	for (int j = 0; j < nBoids; j++)
	{
		if (j == i || distanceBoids(i, j) > r_rule2) continue;
		float V2[3] = {0.0, 0.0, 0.0};
		for (int k = 0; k < 3; k++)
			V2[k] = Boid_Location[j][k] - Boid_Location[i][k];
		for (int k = 0; k < 3; k++)
			Boid_Velocity[i][k] -= k_rule2 * V2[k];
	}
 ...
 ///////////////////////////////////////////
	...
	for (int j = 0; j < nBoids; j++)
	{
		if (j == i || distanceBoids(i, j) > r_rule3) continue;
		num_of_near_boids++;
		for (int k = 0; k < 3; k++)
			V3[k] += Boid_Velocity[j][k];
	}
	for (int k = 0; k < 3; k++)
		if (num_of_near_boids != 0)
			V3[k] /= num_of_near_boids;
	for (int k = 0; k < 3; k++)
		Boid_Velocity[i][k] += k_rule3 * V3[k];
////////////////////////////////////////////
// Select leaders and let all others follow them
	int idx;
	float min_dist = std::numeric_limits<float>::max();
	for (int j = 0; j < MAX_LEADERS; j++)
	{
		float dist = distanceBoids(i, leader_list[j]);
		if (dist < min_dist) {
			min_dist = dist;
			idx = leader_list[j];
		}
	...
	float V4[3] = {0.0};
	for (int k = 0; k < 3; k++)
		V4[k] = Boid_Location[idx][k] - Boid_Location[i][k];
	for (int k = 0; k < 3; k++)
		Boid_Velocity[i][k] += k_rule1 * V4[k];
 ...
 ///////////////////////////////////////////
	for (int k = 0; k < 3; k++)
		Boid_Velocity[i][k] += velocity_before_update[k] * k_rule0;
 ...
 // of this boid.
 ///////////////////////////////////////////
// update boid i location
	for (int k = 0; k < 3; k++)
	{
		Boid_Location[i][k] += SPEED_SCALE * Boid_Velocity[i][k];
	}
